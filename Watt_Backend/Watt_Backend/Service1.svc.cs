...
namespace Watt_Backend
{
    ...
    public class Service1 : IService1
    {
        ...
        {
            var sysUser = (from u in db.SysUsers
            ...
            if (sysUser != null)
            {
                ...
                if (manager.Equals(sysUser.User_Type))
                {
                    return true;
                }
                else
        ...
        public bool Register(string uName, string uGender, DateTime uDOB, string uUsername, string uPassword)
        {
            string uType;

            var userss = (from u in db.SysUsers
                          where u.Username.Equals(uUsername)
                          select u).FirstOrDefault();
            if (userss == null)
            {
                if (uUsername.Contains("@admin"))
                {
                    uType = "Manager";
                }
                else
